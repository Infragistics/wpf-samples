...
namespace IGPivotGrid.Samples.Organization
{
    public partial class Filtering : SampleContainer
    {
        ...
        private const string datesLevelName = "Dates";
        ...
        private void Filter(object sender, RoutedEventArgs e)
        {
            ...
            int numberOfMonths = Convert.ToInt32(monthsToFilter.SelectedItem);

            FilterDates(DateTime.Today.AddMonths(-numberOfMonths), DateTime.Today);
        }
        ...
        private void FilterDates(DateTime beginning, DateTime ending)
        {
            ...
            if (FindDateViewModel((DataSourceBase)pivotGrid.DataSource) == null)
            {
                CreateViewModel((DataSourceBase)pivotGrid.DataSource);
            }
            ...
            worker.RunWorkerCompleted += (s, e) =>
            {
                ...
                if (allUnchecked)
                {
                    ...
                    isFiltering = false;
                    MessageBox.Show(String.Format(PivotGridStrings.XPG_Filtering_NoData, monthsToFilter.SelectedItem));
                }
                else
                {
        ...
        void WorkerDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            IFilterViewModel fvm = FindDateViewModel(args.DataSource);
        ...
        private IFilterViewModel CreateViewModel(DataSourceBase dataSource)
        {
            foreach (IDimension dim in dataSource.Cube.Dimensions)
            {
                ...
                {
                    if (hierarchy.UniqueName == dateHierarchyUniqueName)
                    {
                        IFilterViewModel fvm = dataSource.GetFilterViewModelFromHierarchy(hierarchy);
                        if (fvm == null)
                            fvm = dataSource.CreateFilterViewModel(hierarchy);
        ...
        private void DrillIn(IFilterMember filterMember, WorkerArguments args)
        {
            ...
            if (filterMember.Member.LevelName == datesLevelName)
            {
        ...
        private void DataSourcePropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                isBusyIndicator.Visibility = this.pivotGrid.DataSource.Processing || isFiltering ?
