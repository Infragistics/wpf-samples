...
{
    public partial class WorkingWithPasswordProtectedFiles : SampleContainer
    {
        public WorkingWithPasswordProtectedFiles()
        {
            InitializeComponent();
        }
        ...
        private void BtnClearData_Click(object sender, RoutedEventArgs e)
        {
            ...
            this.ComboBox_ExcelFormat.IsEnabled = true;
        ...
        private void TestExcel_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (Stream stream = this.GetType().Assembly.GetManifestResourceStream(resourceName))
            {
                try
                {
                    dataWorkbook = Workbook.Load(stream, new WorkbookLoadOptions(txtPswToOpen.Text));
                }

                catch (Infragistics.Documents.Core.InvalidPasswordException)
                {
                    MessageBox.Show(ExcelStrings.ErrMsg_OpenPasswordIsInvalid);
                    return;
                }
            }
            ...
            this.ComboBox_ExcelFormat.IsEnabled = false;
        ...
        private string GetExcelFileName()
        {
            ...
            {
                excelFile = "testPassword" + fileExt;
            }
            else
            {
                excelFile = "testPassword_ja" + fileExt;
            }
        ...
        {
            dataWorkbook.ValidateFileWriteProtectionPassword(txtPswToFileWrite.Text);
            string path = Path.GetTempPath();
            string fileName = path + "TestFile" + GetExcelFileName();
            try
            {
