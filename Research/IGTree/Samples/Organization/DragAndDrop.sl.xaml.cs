...
namespace IGTree.Samples.Organization
{
    public partial class DragAndDrop : SampleContainer
    {
        ...
        void xmlDataProvider_GetXmlDataCompleted(object sender, GetXmlDataCompletedEventArgs gxdcea)
        {
            ...
            if (doc != null)
            {
                var result = this.GetGroups(doc.Root, "Contacts");
                this.LoadItems(this.dataTree.Items, result);

                if (result.Count > 0)
                {
                    this.dataTree.XamTreeItems[0].IsExpanded = true;
                    this.dataTree.XamTreeItems[1].IsExpanded = true;
                    this.dataTree.XamTreeItems[2].IsExpanded = true;
                }
            }
        }
